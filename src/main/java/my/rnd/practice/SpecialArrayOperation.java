...
public class SpecialArrayOperation {
	...
	public static void main(String[] args) {
		...
		try {
				...
				List<Map> testCasesList = new ArrayList(); 
				
				for(long testCaseIndex = 0; testCaseIndex < testCaseNo; ++testCaseIndex) {
					...
					testCaseMap.put(VALUES_LIST, valuesList);
					testCaseMap.put(OPERATIONS_LIST, kOperationsList);
					testCasesList.add(testCaseMap);
				}
				
				in.close();
				
				if( testCasesList != null && !testCasesList.isEmpty() ) {
					for(Map testCaseMap : testCasesList) {
						findTheMagicValue((List<Long>)testCaseMap.get(VALUES_LIST), (List<Long>)testCaseMap.get(OPERATIONS_LIST));
					}
				}
		} catch( Exception e) {
			e.printStackTrace();
